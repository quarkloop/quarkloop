...
func TestMutationCreateWorkspace(t *testing.T) {
	...
	t.Run("create workspace with duplicate scopeId", func(t *testing.T) {
		...
		}
		{
			...
			require.Error(t, err)
			require.Exactly(t, wsErrors.ErrWorkspaceAlreadyExists, err)
			require.Equal(t, "workspace with same scopeId already exists", err.Error())
		}
		{
			// clean up
			cmd := &DeleteWorkspaceByIdCommand{OrgId: orgId, WorkspaceId: workspaceId}
			deleteErr := store.DeleteWorkspaceById(ctx, cmd)
			require.NoError(t, deleteErr)

			query := &GetWorkspaceByIdQuery{OrgId: orgId, WorkspaceId: workspaceId}
			ws, err := store.GetWorkspaceById(ctx, query)
			require.Nil(t, ws)
			require.Error(t, err)
			require.Exactly(t, wsErrors.ErrWorkspaceNotFound, err)
	...
	t.Run("create workspace without scopeId", func(t *testing.T) {
		for i := 0; i < workspaceCount; i++ {
			...
			{
				// clean up
				cmd := &DeleteWorkspaceByIdCommand{OrgId: orgId, WorkspaceId: ws.Id}
				deleteErr := store.DeleteWorkspaceById(ctx, cmd)
				require.NoError(t, deleteErr)

				query := &GetWorkspaceByIdQuery{OrgId: orgId, WorkspaceId: ws.Id}
				ws, err := store.GetWorkspaceById(ctx, query)
				require.Nil(t, ws)
				require.Error(t, err)
				require.Exactly(t, wsErrors.ErrWorkspaceNotFound, err)
...
func TestQueryGetWorkspaceAfterCreate(t *testing.T) {
	...
	t.Run("get workspace by wrong id", func(t *testing.T) {
		query := &GetWorkspaceByIdQuery{OrgId: orgId, WorkspaceId: 9999999}
		ws, err := store.GetWorkspaceById(ctx, query)
	...
	t.Run("get workspace visibility by id after creation", func(t *testing.T) {
		...
		for _, ws := range wsList {
			query := &GetWorkspaceVisibilityByIdQuery{OrgId: orgId, WorkspaceId: ws.Id}
...
func TestMutationUpdateWorkspace(t *testing.T) {
	...
	t.Run("update workspace with duplicate scope id", func(t *testing.T) {
		...
		{
			// original scope id
			cmd := &UpdateWorkspaceByIdCommand{
				OrgId:       orgId,
...
func TestQueryWorkspaceRelations(t *testing.T) {
	...
	// 	for _, ws := range wsList {
	// 		query := &GetUserAssignmentListQuery{WorkspaceId: ws.Id}
