...
SET 
    ...
    END,
    "orgUserLimit" = CASE
        WHEN @orgUserLimit > 0 THEN @orgUserLimit
        ELSE q."orgUserLimit"
    END,
    "workspacePerOrgLimit" = CASE
        WHEN @workspacePerOrgLimit > 0 THEN @workspacePerOrgLimit
        ELSE q."workspacePerOrgLimit"
    END,
    "projectPerWorkspaceLimit" = CASE
        WHEN @projectPerWorkspaceLimit > 0 THEN @projectPerWorkspaceLimit
        ELSE q."projectPerWorkspaceLimit"
    END,
    "updatedAt" = @updatedAt,
    "updatedBy" = @updatedBy
WHERE 
