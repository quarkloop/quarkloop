...
func TestMutationCreateProject(t *testing.T) {
	...
	t.Run("create project with duplicate scopeId", func(t *testing.T) {
		...
		}
		{
			...
			require.Error(t, err)
			require.Exactly(t, project.ErrProjectAlreadyExists, err)
			require.Equal(t, "project with same scopeId already exists", err.Error())
		}
		{
			// clean up
			cmd := &project.DeleteProjectByIdCommand{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: projectId}
			deleteErr := store.DeleteProjectById(ctx, cmd)
			require.NoError(t, deleteErr)

			query := &project.GetProjectByIdQuery{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: projectId}
			pr, err := store.GetProjectById(ctx, query)
			require.Nil(t, pr)
			require.Error(t, err)
			require.Exactly(t, project.ErrProjectNotFound, err)
	...
	t.Run("create project without scopeId", func(t *testing.T) {
		for i := 0; i < projectCount; i++ {
			...
			{
				// clean up
				cmd := &project.DeleteProjectByIdCommand{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: pr.Id}
				deleteErr := store.DeleteProjectById(ctx, cmd)
				require.NoError(t, deleteErr)

				query := &project.GetProjectByIdQuery{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: pr.Id}
				pr, err := store.GetProjectById(ctx, query)
				require.Nil(t, pr)
				require.Error(t, err)
				require.Exactly(t, project.ErrProjectNotFound, err)
...
func TestQueryGetProjectAfterCreate(t *testing.T) {
	...
	t.Run("get project by wrong id", func(t *testing.T) {
		query := &project.GetProjectByIdQuery{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: 9999999}
		pr, err := store.GetProjectById(ctx, query)
	...
	t.Run("get project visibility by id after creation", func(t *testing.T) {
		...
		for _, pr := range prList {
			query := &project.GetProjectVisibilityByIdQuery{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: pr.Id}
...
func TestQueryWorkspaceRelations(t *testing.T) {
	...
	t.Run("get project's user assignment list", func(t *testing.T) {
		...
		for _, pr := range prList {
			query := &project.GetUserAssignmentListQuery{OrgId: orgId, WorkspaceId: workspaceId, ProjectId: pr.Id}
...
func TestCleanup(t *testing.T) {
	...
	t.Run("delete workspace by id", func(t *testing.T) {
		...
		err := workspaceStore.DeleteWorkspaceById(ctx, cmd)
		require.NoError(t, err)
	})
